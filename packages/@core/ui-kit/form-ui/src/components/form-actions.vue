...
async function handleReset(e: Event) {
  ...
  if (isFunction(props.handleReset)) {
    ...
    // Kiểm tra form có tồn tại và có method resetForm không
    if (form && typeof form.resetForm === 'function') {
      form.resetForm();
    } else if (props.formApi && typeof props.formApi.resetForm === 'function') {
      await props.formApi.resetForm();
    } else {
      console.warn('Form reset method is not available');
    }
  }
}
